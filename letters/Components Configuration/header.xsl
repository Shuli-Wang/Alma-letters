    ...
    <table cellspacing="0" cellpadding="5" border="0">
      <xsl:attribute name="style">
        ...
        <xsl:choose>
          ...
          <xsl:otherwise>
            <!-- AFN TODO -->
            <xsl:call-template name="headerTableStyleCssLUL" />
            <!-- style.xsl -->
            <!-- END OF AFN TODO -->
          </xsl:otherwise>
        </xsl:choose>
      ...
      <tr>
        <xsl:attribute name="style">
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              <!-- AFN TODO -->
              <xsl:call-template name="headerLogoStyleCssLUL" />
              <!-- style.xsl -->
              <!-- END OF AFN TODO -->
            </xsl:otherwise>
          </xsl:choose>
        ...
        <td colspan="2">
          <div id="mailHeader">
            <div id="logoContainer" class="alignLeft lul-logo-container">
              <!-- AFN CODE -->
              <xsl:choose>
                <xsl:when test="(notification_data/user_for_printing/user_group = 'AFNUSER') or (notification_data/user/user_group = 'AFNUSER') or (notification_data/request/user_group = 'AFNUSER') or (notification_data/user_for_printing/user_group = 'TUGUSER') or (notification_data/user/user_group = 'TUGUSER') or (notification_data/request/user_group = 'TUGUSER')">
                ...
                <xsl:otherwise>
                  <!-- AFN TODO -->
                  <img src="cid:logo.jpg" alt="Lakehead logo" width="300" height="auto" />
                  <!-- END OF AFN TODO -->
                </xsl:otherwise>
              </xsl:choose>
              <!-- END OF AFN CODE -->
